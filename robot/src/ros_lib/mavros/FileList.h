...
static const char FILELIST[] = "mavros/FileList";
  ...
  class FileListResponse : public ros::Msg
  {
    public:
      uint8_t list_length;
      mavros::FileEntry st_list;
      mavros::FileEntry * list;
      bool success;
      int32_t r_errno;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(list_lengthT > list_length)
        this->list = (mavros::FileEntry*)realloc(this->list, list_lengthT * sizeof(mavros::FileEntry));
      offset += 3;
      list_length = list_lengthT;
      for( uint8_t i = 0; i < list_length; i++){
      offset += this->st_list.deserialize(inbuffer + offset);
        memcpy( &(this->list[i]), &(this->st_list), sizeof(mavros::FileEntry));
      }
      union {
        bool real;
        uint8_t base;
